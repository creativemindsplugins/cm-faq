...
abstract class Settings {
	...
	/**
	 * Set value of the specific option
	 ...
	 * @param string $option_name
	 * @param mixed $default
     * @param mixed $skip_config
	 * @return mixed
	 */
	public static function get($option_name, $default = null, $skip_config = false) {


		$all_option = get_option(static::abbrev('_options'), []); //array of all plugin options

        // return value of the option without loading settings config
        if($skip_config) {
	        if(!empty($all_option) && isset($all_option[$option_name])) {
		...
		if ($settingsTabsArray) {
			foreach ($settingsTabsArray as $tabKey => $tabSpecifications) {
				...
				if (!empty($tabContent)) {
					...
					$content .= str_contains($tabContent,'section-title') ? '<div class="cminds_settings_toggle_tabs cminds_settings_toggle-opened">Toggle All</div>' : '';
					$content .= $tabContent;
					$content .= '</div>';
				}
			}
	...
	public static function renderSetting($key, $config) {
		...
		?>
        <tr valign="top"
            class="<?php echo $class; ?> <?php echo $key; ?> wrapper-<?php echo esc_attr($config['type']); ?>" <?php self::setHidden($config); ?>>
            <th scope="row">
	...
	public static function renderOnlyin($onlyin = 'Pro') {
		...
		if (!$renderOnce):
			?>
            <style>
                .onlyinpro:not(.full-width) *{
                    color: #aaa !important;
                }
                .onlyinpro:not(.full-width) {
                    color: #aaa !important;
                }
                .onlyinpro.hide, tr:has(.hide) {
	...
	public static function getOptionsWithCondition() {
		$options = static::config('settings');
		...
		foreach ($options as $key => $option) {
			if(isset($option['condition']) && is_array($option['condition']) && !empty($option['condition'])) {
				...
				if(count($option['condition']) > 1) {
					foreach ($option['condition'] as $conditionKey => $conditionValue) {
						$option_name = $conditionKey;
						$config = self::getConfig($option_name);
					...
					$option_name = array_key_first($option['condition']);
					$config = self::getConfig($option_name);
					$conditionOption[] = [
						'name'  => $option_name,
						'value' => $option['condition'][ $option_name ],
						'type'  => $config['type']
					];
		...
		return $options_with_condition;
	}

	public static function checkOptionCondition($option_config) {
		if(!isset($option_config['condition'])) {
			return;
		}

		$condition = $option_config['condition'];
		if(is_array($condition) && !empty($condition)) {
			foreach ($condition as $key => $optionValue) {
				$handledValue = static::get($key);

				if(is_array($optionValue)) {
					foreach ($optionValue as $valueItem) {
						if($handledValue == $valueItem) {
							return false;
						}
					}
				}
				if($handledValue != $optionValue) {
					return true;
				}
			}
			return false;
			...
			throw new \Exception('Wrong condition ' . $option_config['label']);
		}
	}

	public static function setHidden($option_config) {
		if(self::checkOptionCondition($option_config)) {
	...
	public static function enqueueAssets($page) {
		...
		wp_localize_script( self::abbrev('settings-js'), 'Settings', [
			'optionsWithCondition' => self::getOptionsWithCondition()
		] );
