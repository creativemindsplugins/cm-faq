...
class CMFAQ_Base {
    ...
    public static function init() {
		...
		add_filter( "manage_edit-cmfaq_category_columns", array(__CLASS__, 'custom_column_header'), 10);
		add_action( "manage_cmfaq_category_custom_column", array(__CLASS__, 'custom_column_content'), 10, 3);
    }

	public static function custom_column_header( $columns ) {
		unset($columns['posts']);
		$columns['posts_count'] = 'Count'; 
		return $columns;
	}

	public static function custom_column_content( $value, $column_name, $tax_id ) {
		$out = '';
		$taxonomy = get_term_by('id', $tax_id, 'cmfaq_category');
		$url = admin_url('edit.php?post_type=cmfaq_question&cmfaq_category='.$taxonomy->slug);
		$out = '<a href="'.$url.'">'.$taxonomy->count.'</a>';
		return $out;
	}
    ...
    public static function template_redirect() {
		...
		if(false === get_transient('cmfaqf_refresh_404_permalinks')) {
			$slug = \CM\CMFAQ_Settings::get('cmfaq_question_slug', CMFAQ_SLUG_NAME . '-question');
		    $parts = explode('/', $wp->request);
			if($slug !== $parts[0]) {
				return;
			}
			flush_rewrite_rules(false);
			set_transient('cmfaqf_refresh_404_permalinks', 1, HOUR_IN_SECONDS * 12);
    ...
    public static function the_content_post_simple($content) {
        ...
        $content = wp_trim_words($content, \CM\CMFAQ_Settings::get('cmfaq_num_words', 200));
        self::enqueue_script();
        self::enqueue_style();
        ob_start();
    ...
    private static function add_inline_style() {
        ...
        if ( $style == "" ) {
            ob_start();
            include CMFAQ_PLUGIN_DIR_PATH . 'views/frontend/parts/style.php';
            $style .= ob_get_clean();
            wp_add_inline_style('cmfaq-frontend-style', $style);
        }
    }
    ...
    public static function title_like_posts_where($where, \WP_Query $wp_query) {
        global $wpdb;
    ...
    public static function title_or_content_like_posts_where($where, \WP_Query $wp_query) {
        global $wpdb;
    ...
    public static function pre_get_posts(&$wp_query_ref) {
        ...
        if ($is_tax || $is_search) {
            ...
            if (\CM\CMFAQ_Settings::get('cmfaq_page_id', 0) && get_post(\CM\CMFAQ_Settings::get('cmfaq_page_id', 0)) && ($is_tax || $is_search)) {
                ...
                self::$wp_query_page = new WP_Query(array('page_id' => \CM\CMFAQ_Settings::get('cmfaq_page_id', -1)));
            }
        }
    }
    ...
    public static function create_post_type() {
        register_post_type('cmfaq_question', array(
            ...
            'has_archive' => false,
            'query_var' => true,
            'show_in_rest' => true, // for Guttenberg compability
            'rewrite' => array('slug' => \CM\CMFAQ_Settings::get('cmfaq_question_slug', CMFAQ_SLUG_NAME . '-question'), 'with_front' => false, 'feeds' => true, 'feed' => true),
            'supports' => array('title', 'editor', 'author', 'custom-fields'),
            'taxonomies' => array('cmfaq_category')
        ));

        register_taxonomy('cmfaq_category', 'cmfaq_question', array(
            'label' => 'FAQ Categories',
            'rewrite' => array('slug' => \CM\CMFAQ_Settings::get('cmfaq_category_slug', CMFAQ_SLUG_NAME . '-category')),
            'show_ui' => true,
