...
class CMFAQ_Frontend {
    ...
    public static function the_breadcrumb() {
        ...
        $items = array();
        // Home link
        if (\CM\CMFAQ_Settings::get('cmfaq_page_id', 0) && $item = get_post(\CM\CMFAQ_Settings::get('cmfaq_page_id', 0))) {
            $items[] = '<a class="cmfaq-breadcrumb-link" href="' . esc_attr(get_permalink($item)) . '">' . \CM\CMFAQ_Settings::get('cmfaq_label_breadcrumb_home', 'Home') . '</a>';
        }
        // Post category link
    ...
    public static function the_search() {
        if (! \CM\CMFAQ_Settings::get('cmfaq_show_search_bar')) {
            return;
        }
        include CMFAQ_PLUGIN_DIR_PATH . 'views/frontend/parts/search.php';
    }
    ...
    public static function categories() {
        ...
        if ($html):
            ...
            $label = \CM\CMFAQ_Settings::get('cmfaq_label_side_categories', 'Categories');
            if (strlen($label)) :
    ...
    public static function tag_cloud() {
        ...
        $label = \CM\CMFAQ_Settings::get('cmfaq_label_side_tags', 'Tags');
        if (strlen($label)):
